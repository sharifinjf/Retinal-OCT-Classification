...
FeatureVector = cell(Number_Case,1);
for iter_case = 1:Number_Case
    for iter_B_Scan = 1: Number_B_Scan
        display(['case number ' num2str(iter_case) ' Bscan ' num2str(iter_B_Scan)])
        % -----------------------------------------------------------------
        % Reading the data:
        % -----------------------------------------------------------------
        params_data.X = get_oct_data_BScan(iter_case,iter_B_Scan);
        % -----------------------------------------------------------------
        ...
        FeatureVector{iter_case,1}((iter_B_Scan-1)*length(temp)+1:(iter_B_Scan)*length(temp)) = temp;
        
    end
end
save(['FeatureVector_' num2str(Feature) '.mat'], 'FeatureVector','-v7.3')
...
% Finally, 128 *   =     is the size of each row vector.
