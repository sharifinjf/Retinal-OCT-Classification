...
Number_Test    = 30;    % Total number of Patients in the dataset used for this demo
...
tic;
FeatureVector_Test = cell(Number_Test,1);
for iter_case = 1:Number_Test
    for iter_B_Scan = 1: Number_B_Scan
        display(['case number ' num2str(iter_case) ' Bscan ' num2str(iter_B_Scan)])
        % -----------------------------------------------------------------
        % Reading the data:
        % -----------------------------------------------------------------
        params_data.X = get_oct_data_Test_BScan(iter_case,iter_B_Scan);
        % -----------------------------------------------------------------
        ...
        FeatureVector_Test{iter_case,1}((iter_B_Scan-1)*length(temp)+1:(iter_B_Scan)*length(temp)) = temp;
        
    end
end
Total_Time = toc;
save(['FeatureVector_Test_' num2str(Feature) '.mat'], 'FeatureVector_Test','Total_Time','-v7.3')
